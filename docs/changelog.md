...
### 变更
- **根本原因**：之前的方案用 JavaScript 动态渲染卡片，但预览环境/浏览器缓存导致 JS 不执行，卡片无法显示
- **改为静态HTML**：用 Python 脚本将 courseCases 数据预渲染为静态 HTML，直接写入页面，不再依赖 JS 渲染
- **精简JS**：JavaScript 只保留"展开/收起"和"行业筛选"两个交互功能，用 IIFE 封装避免全局污染
- **验证**：44张卡片（5精选 + 39全部）均已生成静态 HTML，即使 JS 被禁用卡片也能正常显示
...
## 2026-07-16 修复课程案例卡片无法渲染的问题

### 变更
- **修复 JS 语法错误**：courseCases 数组中多个模块/亮点字符串包含中文引号（""），与 JavaScript 字符串的双引号定界符冲突，导致整段脚本解析失败
- **统一改为单引号字符串**：将 courseCases 数组内所有字符串从双引号改为单引号，中文引号保留在内容中不再冲突
- **验证**：精选案例、全部案例、行业筛选、展开/收起交互均恢复正常
